...
<body>
	...
	<div class="w3-border-bottom w3-border-green">
		<c:import url="/nav.do?menu=${categoryIdx}" />
	</div>
	<div>
		<img class="block-center" alt="" src="./image/jane2.png">
		<section class="block-center w3-container">
			<div class="w3-text-grey" id="jeans">
				<p>${fn:length(adminVOs)} items</p>
			</div>
			<ul id="product-list" class="w3-grayscale">
				<c:forEach var="adminVO" items="${adminVOs}">
				<li>
					<a href="productOrder.do?idx=${adminVO.idx}">
						<img src="<c:url value='/image/${adminVO.imageName}' />">
						<p>
							${adminVO.name}<br> 
							<b><fmt:formatNumber value="${adminVO.price}" pattern="#,###" />원</b>
						</p>
					</a>
				</li>
				</c:forEach>
